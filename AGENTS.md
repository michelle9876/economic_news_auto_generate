...
| 직원 | 파일 경로 | 역할 |
|------|----------|------|
| 리서처 | `.Codex/staff/experts/researcher.md` | 데이터 통합·요약, 국내 WebSearch |
| 애널리스트 | `.Codex/staff/experts/analyst.md` | 인사이트·해설 작성 |
...
```bash
python3 .Codex/scv/collect_market_data.py
python3 .Codex/scv/collect_macro_data.py
python3 .Codex/scv/collect_news.py
```
...
`.Codex/staff/experts/researcher.md`를 읽고 리서처 역할을 수행한다.
...
`.Codex/staff/experts/analyst.md`를 읽고 애널리스트 역할을 수행한다.
...
```bash
python3 .Codex/scv/validate_report.py --date YYYYMMDD --type [morning|closing]
```
...
```bash
python3 .Codex/scv/save_to_obsidian.py --date YYYYMMDD --type [morning|closing]
```
